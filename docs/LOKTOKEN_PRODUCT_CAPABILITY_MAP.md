...
| 限流与资源保护 | B | Redis 共享滑动窗口限流 | 公网网关/WAF 使用入口级限流 |
...
| 供应商账单核验 | A/B | 归一化 JSON 导入，按供应商请求 ID 逐笔核对 Token、成本及未匹配记录；支持按模型/渠道汇总收入、成本和毛利 | 需为各供应商补导出格式适配器并完成月结 UAT |
...
| 运营告警 | A/B | 告警事件持久化、指纹去重、首次/重开/恢复 Webhook 通知和失败重试 | 需接入真实通知端、值班规则并完成投递 UAT |
...
| 模型管理 | 模型、渠道、价格 | 批量接入、官方价格核验、利润率自动定价、手工覆盖、健康检查、预检、上架 | 可调用模型目录 |
...
## 8.1 P0 生产网关门禁（2026-08-20）

本轮已落地的基础门禁：

- 余额、订单、福利、价格、消费和账本金额列统一使用 PostgreSQL `BIGINT`，避免微元金额在约 2147 元处溢出。
- Usage 记录保存供应商成本、供应商请求 ID、实际渠道、缓存命中/未命中 Token、推理 Token、原始 usage 和路由尝试。
- `/v1/chat/completions` 接受常用 OpenAI 参数，包括 `max_completion_tokens`、`response_format`、`tools`、`tool_choice`、`top_p`、`stop`、`seed`、惩罚项和推理参数，并向兼容供应商透传。
- `/v1/models` 返回上下文、模态、价格和支持参数元数据；模型详情仍只返回已发布且可调用的模型。
- `/metrics` 暴露最小 Prometheus HTTP 请求计数；Nginx 示例增加共享入口限流，应用内限流继续作为 API Key 级保护。
- 管理控制台渠道配置支持填写供应商输入/输出成本，用于请求级毛利核算。

仍需外部环境完成的 P0 放行条件：

- 生产 PostgreSQL 执行 `0016_billing_ledger_bigint_and_route_metadata` 并完成备份、恢复和回滚演练。
- DeepSeek、Qwen、GLM/Kimi 至少三家真实供应商密钥、价格、健康检查、普通/流式调用和失败切换证据；本地没有对应密钥时不得标记为已验证。
- 正式 LokSystem OIDC、安全通知 Webhook、HTTPS 域名和 Nginx 共享限流配置。
- 公开售卖前接入真实支付沙箱、异步回调幂等、退款和财务对账。
...
3. 真实支付沙箱、退款、发票和财务对账。
4. 将已实现的 `/admin/alerts`（含 Key 到期预警）接入通知 Webhook、值班规则和告警收敛。
