...
用户中心提供“通知中心”，会展示账户登录、安全事件、余额预警和 API Key 到期提醒，支持单条或全部标记已读。通知读取时按账户隔离，运营提醒使用 24 小时去重窗口，不会因重复刷新页面持续生成相同通知。
...
注册或登录返回 `usr_...` 用户会话令牌；管理员试用链接返回 `trl_...` 试用令牌。两者都通过 `Authorization: Bearer <token>` 访问用户中心，但权限范围不同：正式会话可使用完整账户、组织、充值和安全管理能力；试用会话只能访问基础门户、模型和当前试用链接创建的 Key，不能读取或操作正式 Key、组织成员、账户安全、充值订单或兑换福利。每个试用链接都有独立的随机指纹，不同试用链接之间的 Key 和用量互相隔离；试用 Key 的有效期也不会超过试用到期时间。会话令牌和试用令牌均有服务端签名和过期时间，账户停用后立即失效。
