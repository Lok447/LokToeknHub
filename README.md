...
管理控制台的“模型管理”支持供应商级接入。进入 DeepSeek、Qwen、GLM、Kimi、MiniMax 或 Doubao 卡片，填写一次厂家 API 地址、密钥（或服务器环境变量）即可读取厂家真实 `/models` 目录，为该系列候选模型复用同一个加密连接并更新渠道健康状态。接入弹窗中的“批量默认输入/输出售价”是可选兜底值，仅用于没有官方参考价的新模型，不能代表该供应商所有模型的最终售价；同步后必须在模型列表的“定价”入口逐个维护 LokToken 面向用户的输入/输出价格。供应商采购成本在“渠道”中独立维护，不能用平台售价替代。自动发布默认关闭，运营人员应完成真实预检、采购成本核对和价格审核后再单独发布；重复提交会执行同步而不是重复创建模型，已存在的单模型密钥也会在首次供应商同步时平滑迁移。图像、视频模型仍会保留在目录中，直到对应统一调用适配器和计费规则上线。

供应商卡片还提供上游余额管理。DeepSeek 使用官方余额 API 查询；Qwen、GLM、Kimi、MiniMax、Doubao 等供应商若当前没有可验证的公开余额 API，页面会明确显示“需控制台查询”，运营人员可从厂家控制台读取余额后手工录入。余额快照包含金额、币种、来源、查询时间和错误状态，可设置采购预警阈值；上游余额与 LokToken 用户额度完全隔离，不会被模型健康检查推断。
